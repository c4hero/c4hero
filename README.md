...
- **Docs and decisions live with the model.** Structurizr's `!docs` and `!adrs` folders are read, not just preserved: the inspector shows an element's documentation and architecture decision records (adr-tools style or [OKF](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md) frontmatter), superseded decisions are marked, and *New decision* writes a numbered record — or saves one the AI drafted — straight into the folder.
...
- **Share without friction.** Export PNG or SVG, copy straight to the clipboard, or export the whole workspace as one self-contained interactive HTML file — every view, browsable offline, no dependencies, deterministic so it diffs in review. For agents and docs tools, export it instead as an [OKF](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md) knowledge bundle: one markdown file per element and view, cross-linked, ready for a knowledge base or an AI assistant to index.
