...
export interface WorkspaceState extends UndoState {
  ...
  /** Append a `!` directive line (`!docs docs/api`) inside an element's block.
   *  No-op when the element already carries that exact line. Undoable. */
  addElementDirective: (id: string, raw: string) => void
  /** Append a workspace-scope `!` directive line (`!adrs decisions`).
   *  No-op when present. Undoable. */
  addWorkspaceDirective: (raw: string) => void
  ...
  /** Workspace documentation dialog (TEA-336): the `!docs` / `!adrs` bundles. */
