...
**Goal:** Replace the mode-tab spotlight with a single inline row of four facet controls (Tags, Status, Tech, Teams) that share one match helper, one highlight semantic, and a collapsible bar — eliminating dim-everything-else behavior.

**Architecture:** All filter facets become `string[]` in the store (Tags/Status/Tech already exist; Team is new). One pure helper (`src/lib/spotlight.ts`) computes `isSpotlit` from `(element, filters)` with AND-across-facets and per-facet within-semantics (Tags/Status/Teams = OR, Tech = AND). `Canvas.buildNodes` / `buildEdges` use the helper to set a single `data.spotlit` boolean; CSS classes `c4-node-spotlit` / `c4-edge-spotlit` (renamed from `*-tech-highlight`) render the rail/glow. The bottom strip becomes four `<FacetControl>` instances around a shared popover, with manual + auto collapse to a compact pill.

**Tech Stack:** TypeScript, React 18, Zustand store, React Flow v12, Vitest (unit), Playwright (e2e).

**Spec:** `docs/superpowers/specs/2026-05-03-spotlight-redesign-design.md`
...
**New files:**
- `src/lib/spotlight.ts` — pure match helpers
- `src/lib/spotlight.test.ts` — vitest unit tests
- `src/components/layout/spotlight/FacetControl.tsx` — generic facet popover
- `src/components/layout/spotlight/SpotlightBar.tsx` — orchestrator (replaces today's filter region of FloatingBottomStrip)
- `src/components/layout/spotlight/useSpotlightCollapsed.ts` — localStorage-backed collapsed state hook
- `e2e/spotlight/spotlight.spec.ts` — multi-facet stacking + collapse e2e
...
- `src/components/canvas/nodes/BaseC4Node.tsx` — `data.spotlit` + reason instead of `activeTagFilter` lookup
- `src/components/canvas/edges/RelationshipEdge.tsx` — read `data.spotlit`
- `src/components/canvas/nodes/types.ts` — node data type updates
- `src/index.css` — class renames + spotlit visuals
- `src/components/layout/FloatingBottomStrip.tsx` — delegate filter UI to `<SpotlightBar>`; tag manager stays here
...
## Task 1: Spotlight match helper + tests

**Files:**
- Create: `src/lib/spotlight.ts`
- Create: `src/lib/spotlight.test.ts`

- [ ] **Step 1: Write the failing tests**

```ts
// src/lib/spotlight.test.ts
...
Run: `npx vitest run src/lib/spotlight.test.ts`
...
```bash
git add src/lib/spotlight.ts src/lib/spotlight.test.ts
git commit -m "feat(spotlight): add pure match helper for facet filtering"
```
...
git commit -m "feat(spotlight): migrate filter state to arrays, add team filter"
```

---

## Task 3: Wire Canvas to spotlight helper, drop opacity dim, rename CSS classes

**Files:**
- Modify: `src/components/canvas/Canvas.tsx:163-237, 403-519, 533-545, 614-672`
- Modify: `src/components/canvas/nodes/types.ts`
- Modify: `src/index.css:1560-1586` (rename `.c4-tech-marquee` → `.c4-node-spotlit-rail`, `.c4-edge-tech-highlight` → `.c4-edge-spotlit`)
- Modify: `src/components/canvas/nodes/BaseC4Node.tsx:120` (read `data.spotlit` + use new class)
- Modify: `src/components/canvas/edges/RelationshipEdge.tsx` (read `data.spotlit`)
...
In `src/components/canvas/Canvas.tsx`, replace the `buildNodes` body's filter section (lines ~163-237) with one that reads `SpotlightFilters` instead of three separate filters and calls `isSpotlit`. New signature:
...
function buildNodes(
  ...
  for (const viewEl of view.elements) {
    ...
    nodes.push({
      ...
      // No more opacity dim. Spotlit nodes get the rail/glow class; non-matches render normally.
...
Delete the previous `effectiveTagFilter` / `effectiveStatusFilter` "auto-suppress" logic — it's no longer needed: `isSpotlit` already returns false for elements that don't match, and a facet that matches nothing in the view simply means no node gets the rail. (The "show count `(0)` for facets with no match in view" UX moves to Task 5.)

- [ ] **Step 3: Replace buildEdges filter logic**

Same file, in `buildEdges` (around line 403-519), replace the per-edge tech matching with `isSpotlitRel`:
...
In `Canvas.tsx` around line 533, the component reads three filter values. Replace with four reads + a memoized `SpotlightFilters` object:
...
Update the `buildNodes` and `buildEdges` calls inside the `useMemo` (lines ~636 and ~649) to pass `spotlightFilters`. Update that `useMemo`'s deps array to use `spotlightFilters` instead of the three previous filter values.

- [ ] **Step 5: Update CSS class names**

In `src/index.css`:
- Rename `.c4-tech-marquee` → `.c4-node-spotlit-rail` everywhere it appears (selector + person variant).
- Rename `.c4-edge-tech-highlight` → `.c4-edge-spotlit` everywhere it appears.
...
- [ ] **Step 6: Update BaseC4Node to read data.spotlit**
...
Also: replace the `activeTagFilter` lookup block (lines 58 + 216-240) that renders `c4-node-tag-label`. New behavior: render the under-node label only when the node is `data.spotlit`. The label text comes from a small helper that reads from the spotlight filters and picks the most-specific reason. Add to BaseC4Node:
...
const reasonLabel = data.spotlit ? pickSpotlitReason(element, filters) : null
```
...
Define `pickSpotlitReason` as a local helper at the bottom of the file:
...
(Imports: `import type { ModelElement } from '@/types/model'`, `import type { SpotlightFilters } from '@/lib/spotlight'`.)

Rename the CSS class `c4-node-tag-label` → `c4-node-spotlit-label` in `index.css` and the JSX.

- [ ] **Step 7: Update RelationshipEdge to read data.spotlit**

In `src/components/canvas/edges/RelationshipEdge.tsx`, find any read of `data.techHighlighted` and rename to `data.spotlit`. (If the edge component currently doesn't render a marquee, no visual change is needed — the className `c4-edge-spotlit` is applied at the React Flow edge level by `buildEdges` and the CSS already targets `.c4-edge-spotlit path.react-flow__edge-path`.)
...
1. `npm run dev`
2. Open a workspace, open the spotlight, toggle a tag.
...
```bash
git add src/lib/spotlight.ts src/components/canvas/Canvas.tsx src/components/canvas/nodes/BaseC4Node.tsx src/components/canvas/nodes/types.ts src/components/canvas/edges/RelationshipEdge.tsx src/index.css
git commit -m "feat(spotlight): drop opacity dim, route all facets through isSpotlit helper"
```
...
**Files:**
- Create: `src/components/layout/spotlight/FacetControl.tsx`
...
```tsx
// src/components/layout/spotlight/FacetControl.tsx
...
```bash
git add src/components/layout/spotlight/FacetControl.tsx
git commit -m "feat(spotlight): add generic FacetControl popover"
```

---

## Task 5: SpotlightBar orchestrator + collapse hook

**Files:**
- Create: `src/components/layout/spotlight/useSpotlightCollapsed.ts`
- Create: `src/components/layout/spotlight/SpotlightBar.tsx`

- [ ] **Step 1: Collapse state hook**

```tsx
// src/components/layout/spotlight/useSpotlightCollapsed.ts
...
```tsx
// src/components/layout/spotlight/SpotlightBar.tsx
...
```bash
git add src/components/layout/spotlight/SpotlightBar.tsx src/components/layout/spotlight/useSpotlightCollapsed.ts
git commit -m "feat(spotlight): add SpotlightBar orchestrator with collapse"
```

---

## Task 6: Replace FloatingBottomStrip filter UI with SpotlightBar; mark canvas chrome
...
2. Remove the inline Tags / Status / Tech filter bodies (the `mode === 'tags'`, `mode === 'status'`, `mode === 'tech'` JSX blocks) — they're replaced by `<SpotlightBar />`.
3. Remove `TechFilterControls` and the `STATUS_OPTIONS` constant (now in `SpotlightBar`/`FacetControl`).
4. Keep the `TagManagerPanel` and `ScopeViolationBanner` parts — they're not replaced.

The outer file becomes a thin shell that wraps `<SpotlightBar />` with the existing fixed-bottom positioner and renders the manage-tags pencil button + portal alongside.
...
export default function FloatingBottomStrip() {
  ...
  return (
    <>
      ...
      >
        <SpotlightBar />
        {/* Pencil button to open the tag manager — kept here so SpotlightBar stays focused on filters. */}
        {/* (Position absolute or inline next to SpotlightBar — pick whichever fits the existing layout.) */}
      </div>
...
If keeping the pencil button proves awkward, accept that as scope: move it inside `SpotlightBar` as a final trailing button next to the collapse chevron, or surface it via the Tags facet popover header (`+ Manage tags`). Default: trailing button next to collapse chevron in the expanded bar.
...
Now clicking a spotlight chip won't dismiss the inspector.

- [ ] **Step 3: Smoke-test**

Run: `npm run dev`
1. Select a node; inspector opens.
2. Click a Tag chip in the spotlight → inspector stays open, node's spotlit ring updates if the tag matches.
...
git commit -m "feat(spotlight): mount SpotlightBar; protect inspector from chip clicks"
```
...
test.describe('spotlight bar', () => {
  ...
  test('AND across facets: matches must satisfy every active facet', async ({ page }) => {
    ...
    // Verify only nodes matching auth + Live carry the spotlit class.
    const spotlit = page.locator('.react-flow__node.c4-node-spotlit')
    await expect(spotlit).toHaveCount(/* fill in expected count from fixture */ 1)
  })
  ...
  test('manual collapse persists across reload', async ({ page }) => {
    await page.getByLabel('Collapse spotlight').click()
    await expect(page.getByRole('button', { name: /Spotlight|active/ })).toBeVisible()
    await page.reload()
    await expect(page.getByRole('button', { name: /Spotlight|active/ })).toBeVisible()
  })
})
...
In `e2e/tags/tag-filtering.spec.ts`, the spotlight no longer has a `Tags` mode tab; instead clicking the Tags facet button opens the popover. Update each interaction:
...
Run: `npx playwright test e2e/spotlight e2e/tags`
Expected: PASS.

- [ ] **Step 4: Commit**

```bash
git add e2e/spotlight/spotlight.spec.ts e2e/tags/tag-filtering.spec.ts
git commit -m "test(spotlight): cover AND-stacking, no-match inactivity, collapse persistence"
```
...
```bash
git add -u
git commit -m "chore(spotlight): remove stale tech-highlight identifiers"
```
...
5. **AND stacking:** add a status, then a tech, then a team. After each, the set of spotlit nodes shrinks (or stays the same), never grows. Hover any spotlit node — under-node label shows the most-specific reason (tech > tag > team > status).
6. **Stale filter:** switch to a view that has no element with the active status → spotlight bar's Status pill shows `1 (0)` and is muted; canvas has no spotlit rings; nothing is dimmed.
