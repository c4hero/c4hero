...
Replace the mode-tab highlighter with a single inline row of four facet controls (Tags, Status, Tech, Teams) that share one selection model, one highlight semantic, and one visual treatment. Eliminate the dim-everything-else behavior.
...
One pure function in a new `src/lib/highlight.ts`:
...
Rules:
- If no facet has any value, `highlightActive` returns false → nothing is highlighted, nothing is dimmed.
...
`Canvas.buildNodes` / `buildEdges` collapse into: compute `highlighted = highlightActive(f) && isHighlighted(el, f)`. Pass `highlighted: boolean` on `node.data` / `edge.data`.

### 3. Visual treatment — drop the dim

- **Highlight ON, node matches:** existing `c4-tech-marquee` rail + `c4-node-tech-highlight` outer glow. Rename CSS class to `c4-node-highlighted` / `c4-edge-highlighted` (keep the visual; rename to reflect generalized purpose).
- **Highlight ON, node does not match:** **no** opacity change, **no** dim. Just absence of the highlighted ring. This is the central UX shift.
...
- Edge: same treatment via `c4-edge-highlighted` (rename of `c4-edge-tech-highlight`).

### 4. Under-node label

Current `c4-node-tag-label` shows the active tag under matching nodes. Generalize: when a node is highlighted and there's a single most-specific reason, render it under the node. Priority: tech > tag > team > status. If multiple reasons tie, show the count (`+2`) — keep it small.
...
Mark the bottom strip's outermost glass panel with `data-canvas-chrome="highlighter-panel"` and update `FloatingInspector.tsx:32` to also exempt `[data-canvas-chrome]` from the outside-click clear. Clicking a facet chip stops closing the inspector.
...
- Replace `c4-edge-tech-highlight` and `c4-tech-marquee` selectors with `c4-edge-highlighted` and `c4-node-highlighted` everywhere.
- Remove `data.dimmed` and `data.techHighlighted` from node data; replace with `data.highlighted`.
...
2. Land `src/lib/highlight.ts` with the match helpers + a unit test file.
...
6. Update `BaseC4Node` under-node label to read from `data.highlighted` + reason. Keep tag-priority default.
...
- **Unit:** new `highlight.test.ts` covers AND-across, OR-within (tags/status/teams), AND-within (tech), empty filters, missing-owner element.
- **Integration:** `workspace.test.ts` migrated — single-select setters delegate correctly; load clears all four filters.
- **E2E:**
  - Apply a tag, status, and tech together; only nodes matching all three show the highlighted rail.
